...
tail(as.matrix(mammals))
str(mammals)

dim(animals)
head(as.matrix(animals))
tail(as.matrix(animals))
str(animals)
...
```{r}
commonAnimals <- intersect(rownames(animals),rownames(mammals))
```
...
setdiff(rownames(mammals),commonAnimals)
setdiff(rownames(animals),commonAnimals)
length(setdiff(rownames(animals),commonAnimals)) + length(setdiff(rownames(mammals),commonAnimals))
#Son animales extintos o de los polos
...
```{r}
#La diferencia es que uno son mamíferos y el otro son animales en general incluyendo dinosaurios
```
...
car::qqPlot(log(animals$brain))
#Como vemos siguen el mismo comportamiento, y siguen una distribución Gaussiana
```
...
library(MASS)
data("cabbages")
car::qqPlot(cabbages$HeadWt)
car::qqPlot(cabbages$VitC)
...
```{r}
hist(cabbages[cabbages$Cult == "c39",4])
hist(cabbages[cabbages$Cult == "c52",4])
```
...
legend(x="topright",legend=c("c39","c52"), pch=c(6,16))
# Planteamos 2 modelos lineales, uno para los datos de cada factor
